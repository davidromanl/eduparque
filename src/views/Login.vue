  ...
  <div id="login">
    <v-container fill-height>
      <v-layout align-center justify-center>
        <v-flex xs12 sm8 md5>
          <v-form v-model="valid" ref="form" @submit.prevent="enviar"
            @keyup.prevent.enter="enviar" lazy-validation>
                ...
                <v-expand-transition>
                  ...
                  <v-checkbox  v-if="tab==1"
                    ...
                    <template v-slot:label>
                      <div>Acepto los <a target="_blank"
                        @click.stop
...
export default {
  ...
  methods: {
    ...
    login() {
      this.$store.dispatch("usuario/login",this.user)
        .then( () => {
          this.$router.push("/")
        } )
        .catch( () => {
    ...
    registro() {
      this.$store.dispatch("usuario/registro",this.user)
        .then( (r) => {
          this.msg_on = true
