<template>
  <v-container class="pa-0" fluid>
    <UploadImage tipo="curso" @upload="upload" />
    <v-hover>
      <template v-slot:default="{ hover }">
        <v-parallax height="300" :src="'/api/img/curso/0?' + rand">
          <v-overlay
            absolute
    ...
    <v-container v-if="isAdmin">
      ...
      <v-row>
        <v-col>
          <h2>Admin</h2>
          <v-btn @click="nuevo" color="primary">
            <v-icon>mdi-plus</v-icon></v-btn
          >
          |
          <v-btn @click="listar" color="success">
                    ...
                    <v-menu>
                      ...
                      <v-card>
                        <v-card-text class="py-1">
                          ...
                          <v-btn @click="eliminar(curso.id)" small color="error"
                            >SI
...
export default {
  name: "Cursos",
  components: { CursoForm, UploadImage },
  ...
  computed: {
    ...
    isAdmin() {
      return this.$store.getters["usuario/isAdmin"];
    },
  },
  ...
  methods: {
    ...
    eliminar(id) {
      this.$store.dispatch("curso/eliminar", id).then(this.listar());
    },
