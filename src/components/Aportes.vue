<template>
  <div>
    <v-img :src="'/api/img/curso/'+$route.params.id"
      gradient="to bottom, rgba(0,0,0,.1), rgba(0,0,0,.5)"
      height="200" class="white--text pa-2 align-end">
        <h2>{{curso.nombre}}</h2>
    ...
    <v-toolbar class="fixed" dense tile>
      ...
      <template v-slot:extension>
        ...
        <v-btn small text outlined
          @click="add_aporte">Enviar {{ tipo[tab] }}
          <v-icon right color="success">mdi-comment-{{(tab)?'question':'plus'}}</v-icon></v-btn>
      </template>
    </v-toolbar>

    <v-tabs-items v-model="tab">
      <v-tab-item  class="amber lighten-4">
        <p class="text-center py-4" v-if="aportes.length==0">No hay aportes</p>
          ...
          <div v-for="aport in aportes" :key="aport.id" class="pa-2">
            <v-card hover class="amber lighten-2">
              <v-card-title class="py-1 amber">
                ...
                <div class="ml-2">{{aport.email}}</div>
                <v-spacer></v-spacer>
      ...
      <v-tab-item  class="amber lighten-3">
        <p class="text-center py-4" v-if="preguntas.length==0">No hay preguntas</p>
        <v-fade-transition group>
        <div v-for="pregu in preguntas" :key="pregu.id" class="pa-3">
          <v-card class="amber lighten-2">
            <v-card-title class="py-1 amber">
              <v-avatar>
                <v-img :src="'/api/img/avatar/'+pregu.id_usuario"></v-img>
              </v-avatar>
              <div class="ml-2">
                <h4>{{pregu.nombre}}</h4>
                <div>{{pregu.email}}</div>
