    ...
    <v-card min-width="320px"
      ...
      <v-form lazy-validation
        ...
        <v-card-title class="amber lighten-1">
            <v-icon left>mdi-comment-{{(aporte.tipo)?'question':'plus'}}</v-icon>
            {{(aporte.id)?'Editar':'Enviar'}} {{ (aporte.tipo)?'Pregunta':'Aporte'}}
            <v-spacer></v-spacer>
        ...
        <v-card-actions>
            ...
            <v-spacer></v-spacer>
            <v-btn :disabled="disable" @click="send_aporte" small
